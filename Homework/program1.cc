...
int main() {
  ...
  cin >> initial_month >> slash >> initial_day >> slash >> initial_year >> initial_balance;
  ...
  while(true){
    ...
    if(month == initial_month) {
    ...
    else if(month == initial_month + 1 || (initial_month == 12 && month == 1)) {
      ...
      if(current_total_days <= 30)
        total_days = ((days_per_month[initial_month - 1] - initial_day) + days_per_month[initial_month] + day + 1);
      else {
...
//Works
    if ((month < current_month && year < current_year) || (day < current_day && month == current_month) || year < current_year) {
      cout << "Entries must be provided in chronological order." << endl;
      cout << month << slash << day << slash << year << " entered after " << current_month << slash << current_day
    ...
    switch (indicator) {
      case 'd':
        ...
        if (current_balance < 0) {
          current_balance -= 15;
          cout << "Overdraft on " << month << slash << day << ". $15 overdraft fee assessed." << endl;
        }
        break;
      case 'c':
        current_balance += amount;
  ...
  if (indicator == 'q') {
    ...
    //cout << sum_total << endl;
    cout << total_days << "-day average balance (" << initial_month << slash << initial_day << slash << initial_year << "-" 
    << month << slash << day << slash << year << "): $" << std::fixed << std::setprecision(2) << average_balance << endl;
  }
...
//Fix
  if (total_days > 30) {
    ...
    if (current_month == initial_month && current_year == initial_year) {
      cout << "30-day average balance (" << initial_month << slash << initial_day << slash << initial_year << "-" 
      << month << slash << (initial_day + 29) << slash << year << "): $" << std::fixed << std::setprecision(2) 
      << average_balance << endl;
    }
    else if (current_month == initial_month + 1 && current_year == initial_year) {
      int new_day;
      if ((year % 400 == 0 || (year % 4 == 0 && year % 100 != 0)) && initial_month == 2)
        new_day = 30 - (30 - initial_day);
      else
        new_day = 30 - ((days_per_month[initial_month - 1] + 1) - initial_day);
      cout << "30-day average balance (" << initial_month << slash << initial_day << slash 
      << initial_year << "-" << current_month << slash << new_day << slash
      << current_year << "): $" << std::fixed << std::setprecision(2)
      << average_balance << endl;
    }
  }
  return 0;
